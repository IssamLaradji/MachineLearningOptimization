...
def select_coordinate(rule, x, A, b, args):
  ...
  if rule == 'cyclic':
      ...
      if epoch == 0:
        ...
        heap, h2l, l2h, n_elements = max_heap.create_heap(scores)
        neighbors, n_neighbors, max_depend = get_neighbors_matrix(A)
...
def get_neighbors_matrix(A):
  ...
  neighbors = np.ones((n_depend, max_depend), "int32") * -1
  n_neighbors = np.zeros(n_params + 1, "int32")

  for i in range(n_params):
      depend = np.where(correlation[i] > 0)[0]

      n_neighbors[i] = depend.size

      neighbors[i, :n_neighbors[i]] = depend
  
  return neighbors, n_neighbors, max_depend
...
#   neighbors = np.ones(n_depend, "int32") * -1
#   n_neighbors = np.zeros(n_params + 1, "int32")

#   for i in range(n_params):
#       depend = np.where(correlation[i] > 0)[0]

#       n_neighbors[i + 1] = n_neighbors[i] + depend.size

#       neighbors[n_neighbors[i]: n_neighbors[i+1]] = depend
